...
- **ADR-013 (2026-05-21, supersedes ADR-006's "Next.js dev server" framing for distribution):** v0.4 distributes as an **npm package via `npx adeptly`**. *Reason:* zero hosting cost for Openesia; users bring their own Claude Code subscription via the local `claude --print` CLI; one-command install matches the workflow of every developer in the audience. Implementation: `next.config.mjs` uses `output: "standalone"`; `scripts/pack-assets.js` copies `.next/static` + `public/` into the standalone bundle; `bin/adeptly.js` is the CLI entry, captures user cwd as ADEPTLY_PROJECT_ROOT, picks a free port, launches the standalone server, opens the browser. Published as `adeptly` on npm (pending). Reddit + Hacker News launch follow once the npm install path is verified.
- **ADR-014 (2026-05-21):** Multi-developer workflow is **git-as-sync, no backend**. Plans live as markdown files under `docs/plans/`. Approval state lives as JSON files alongside. The user's repo is the source of truth. Adeptly surfaces git-level signals (last author from `git log`, dirty/staged/untracked from `git status --porcelain`) per plan. *Reason:* every developer in the target audience already uses git; we get awareness/conflict-resolution/permanence for free; no central server to operate or pay for. Real-time sync (live cursors, in-app notifications) deferred to a paid v1+ tier if traction justifies it.
