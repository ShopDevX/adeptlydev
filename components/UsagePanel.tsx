...
export function UsagePanel({ projectRoot }: { projectRoot: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const q = `?projectRoot=${encodeURIComponent(projectRoot)}${days ? `&days=${days}` : ""}`;
    ...
    <div className="flex flex-col h-full bg-elevated">
      <div className="px-3 py-2 border-b border-border-subtle sticky top-0 bg-elevated z-10">
        ...
        <div className="text-[11px] text-fg-tertiary mt-0.5">
          What Adeptly's own <span className="font-mono">claude</span> calls cost — a local ledger, not your whole bill.
        </div>
      </div>
